...
def stream_file(args):
    ...
    for track in mid.tracks:
        ...
        piano = [
            Note(get_enum_string(i + SEMITONE_SHIFT + 12), 0, 0) for i in range(128)
        ]
        total_time = 0
    ...
    first_start = all_notes[0].time
    for note in all_notes:
        note.time = note.time - first_start
    ...
    for note in all_notes:
        ...
        if earliest_index == -1:
            earliest_start = note.time + 99999
            earliest_index = -1
            for i, motor in enumerate(motor_notes):
                if len(motor) == 0:
                    earliest_index = i
